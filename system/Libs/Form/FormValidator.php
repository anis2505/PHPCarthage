...
/**
* Pork Formvalidator. validates fields by regexes and can sanatize them.
* Uses PHP filter_var built-in functions and extra regexes 
* @package pork
*/
...
class FormValidator
{
    ...
    public $validations, $sanatations, $mandatories, $equal, $errors, $corrects, $fields;
    ...
    public function validate($items)
    {
        ...
        foreach($this->validations as $key=>$val)
        {
                ...
                if(!is_callable($val) && !array_key_exists($key,$items))
                {
                        $this->addError($key, $val);
                        continue;
                }
                if(is_callable($val) && $val instanceof \Closure)
                    $result = call_user_func_array($val,array($items[$key]));
                else
                ...
                if($result === false) {
                    ...
                    if(!is_callable($val))
                        ...
                        $this->addError($key,"custom_validation");
                }
                else
                {
                        $this->corrects[] = $key;
